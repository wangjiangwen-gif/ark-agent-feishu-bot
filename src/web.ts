...
function renderAttachmentRows(items){
  const stages={download:'从飞书下载',upload:'上传到 MA',upload_check:'核查远端上传',mount:'挂载到 Session',mount_check:'核查远端挂载',inline:'准备纯文本',cache:'复用附件缓存'};
...
function auditRunStatus(x){
  const v=x.fileObservation;if(!v)return x.status;
  const timing={after_reads:'读取结果后有回复',before_reads_finished:'读取结束后没有新回复',none:'未观察到文本回复',unknown:'读取与回复顺序待核实'};
  const notes=['观察到 '+v.reads.length+' 次读取，'+v.reads.filter(r=>r.hasDocument).length+' 次文档结果'];
  const errors=v.reads.filter(r=>r.outcome==='error').length,missing=v.reads.filter(r=>r.outcome==='missing_event').length;
  if(errors)notes.push(errors+' 次读取报错');if(missing)notes.push(missing+' 次缺少结果');
  if(v.unmatchedDocumentResults)notes.push(v.unmatchedDocumentResults+' 次未关联文档结果');if(v.truncated)notes.push('观测记录达到上限');
  notes.push(timing[v.replyTiming]||timing.unknown,'业务结果未自动验收');return x.status+' / '+notes.join(' / ');
}
async function loadAudit(){const xs=await api(employeeBase()+'/audit'),body=document.querySelector('#audit-body');body.replaceChildren(...xs.map(x=>{const tr=document.createElement('tr');[new Date(x.createdAt).toLocaleString(),x.openId,x.action,auditRunStatus(x),x.durationMs==null?'—':x.durationMs+' ms',x.sessionId||'—'].forEach(v=>{const td=document.createElement('td');td.textContent=esc(v);tr.append(td)});return tr}));if(!xs.length){const tr=document.createElement('tr'),td=document.createElement('td');td.colSpan=6;td.className='empty';td.textContent='暂无行为日志';tr.append(td);body.append(tr)}}
