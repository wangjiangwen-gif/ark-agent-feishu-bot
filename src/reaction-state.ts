...
import type { ChannelMessage, ReactionObservation } from "./channel.ts";
...
export type ReactionReceipt = { id: string; taskId: string; owner: string; revision: number;
  phase: "creating" | "active" | "removing"; emoji: "Get" | "OnIt"; reactionId?: string; createdAt?: number;
  inspection?: { checkedAt: number; observation: ReactionObservation } };
...
export class ReactionStateStore {
  ...
  begin(message: ChannelMessage, emoji: string): ReactionReceipt {
    ...
    const receipt: ReactionReceipt = { id: randomUUID(), taskId: task.id, owner, revision: 1, phase: "creating", emoji: emoji as "Get" | "OnIt", createdAt: Date.now() };
  ...
  recordInspection(expected: ReactionReceipt, observation: ReactionObservation): ReactionReceipt {
    const current = this.get(expected.id);
    if (current.revision !== expected.revision || current.taskId !== expected.taskId || current.owner !== expected.owner
      || current.phase !== expected.phase || current.reactionId !== expected.reactionId) throw new Error("待核查表情已变化");
    if (!["present", "absent", "unknown"].includes(observation?.status)) throw new Error("表情核查结果无效");
    if (observation.status === "present" && (typeof observation.reactionId !== "string" || !observation.reactionId.trim()
      || observation.reactionId.length > 1024 || (current.reactionId && current.reactionId !== observation.reactionId))) throw new Error("表情核查ID与原记录不一致");
    return this.save(current, { ...current, owner: current.phase === "creating" && observation.status !== "present" ? current.owner : this.runtimeOwner(),
      ...(observation.status === "present" && current.phase === "creating" ? { phase: "active", reactionId: observation.reactionId } : {}),
      inspection: { checkedAt: Date.now(), observation: structuredClone(observation) } });
  }

  finishAbsent(expected: ReactionReceipt): void {
    const current = this.get(expected.id), age = Date.now() - (current.inspection?.checkedAt ?? 0);
    // 未取得ID的发送意图即使暂时查不到也可能迟到生效，不删除它。
    if (current.revision !== expected.revision || current.owner !== this.runtimeOwner() || !current.reactionId || current.phase === "creating"
      || current.inspection?.observation.status !== "absent" || age < 0 || age > 30_000) throw new Error("表情不存在的核查证据不足");
    const result = this.db.prepare("DELETE FROM gateway_reaction_receipts WHERE id=? AND revision=?").run(current.id, current.revision);
    if (Number(result.changes) !== 1) throw new Error("表情核查检查点已变化");
  }

  pending(channelType: string, installationId: string, includeUnknown = false): Array<{ receipt: ReactionReceipt; message: ChannelMessage }> {
    ...
    const rows = this.db.prepare(`SELECT r.* FROM gateway_reaction_receipts r JOIN gateway_message_inbox i ON i.id=r.task_id
      WHERE i.channel_type=? AND i.installation_id=? AND (r.phase IN ('active','removing') OR (?=1 AND r.phase='creating'))
      AND (r.owner<>? OR r.phase='removing' OR i.state IN ('completed','failed','uncertain','awaiting_authorization'))
      ORDER BY i.sequence, r.id`).all(channelType, installationId, includeUnknown ? 1 : 0, owner);
    return rows.map(row => {
  ...
  private encode(r: ReactionReceipt): string {
    return this.credentials.sealAuthorization(JSON.stringify({ emoji: r.emoji, reactionId: r.reactionId, createdAt: r.createdAt, inspection: r.inspection }), this.context(r));
  }
  private decode(row: Record<string, unknown>): ReactionReceipt {
    ...
    try {
      ...
      if (payload.createdAt !== undefined && (!Number.isSafeInteger(payload.createdAt) || payload.createdAt <= 0)) throw new Error("invalid timestamp");
      return { ...metadata, emoji: payload.emoji, ...(payload.reactionId ? { reactionId: payload.reactionId } : {}),
        ...(payload.createdAt !== undefined ? { createdAt: payload.createdAt } : {}), ...(payload.inspection ? { inspection: payload.inspection } : {}) };
