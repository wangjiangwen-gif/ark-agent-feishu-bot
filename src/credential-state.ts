    ...
    try {
      try { fd = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); }
        ...
        const exists = this.db.prepare("SELECT 1 FROM employee_credentials LIMIT 1").get()
          ...
          || (this.db.prepare("SELECT 1 FROM sqlite_master WHERE name='employee_credential_provisioning'").get()
            && this.db.prepare("SELECT 1 FROM employee_credential_provisioning LIMIT 1").get());
