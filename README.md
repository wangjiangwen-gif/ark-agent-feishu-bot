...
上述恢复只覆盖“输入准备完成后”的中断窗口。创建Session结果未知、输入未完整保存、旧OAuth续跑代次、命令任务、MA非空闲、绑定变化或授权等待时，不自动重新准备或派发。派发记录先落库；即使随后在真正调用MA前退出，也仍按未知结果核查，不盲重发。检查点最终输入上限为2MiB（UTF-8），不扩大现有单轮附件/上下文预算。完整准备事务恢复仍待完成，不能作为生产恢复承诺。
...
这一步只证明 **Session 资源是否已创建**，不等于旧业务输入已准备完成。实验队列中 ready 前中断的任务只核实资源，不重新拉取群历史或调用开发者 hook，也不自动派发旧任务。完整准备过程恢复和未决记录的人工处理入口仍待完成。恢复需要数据库配套的加密密钥，不能只复制数据库或删除密钥后重建。
