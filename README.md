...
- 保留单聊复用 Session、群公共会话共享并排队、Thread 独立会话、群聊 Bot-only、单聊按需 OAuth、Get/在做了表情和流式回复。
- 包含逐轮身份与引用上下文、原生 Session 创建配置、文件处理诊断和失败提示修复。网关不再自动发送 `/compact`；只有用户显式输入才提交，平台自身压缩不受影响。
- 正式 CLI 不启用实验持久队列，不提供 Session 升级或动态追加记忆入口；不要把本版当成重启后全部任务自动续跑的承诺。
- PDF 新路径：Gateway 下载并上传 Files API，确认文件 `active` 后，将 `document.source={type:"file",file_id:...}` 直接交给 MA 模型；不是把 URL 当普通文本发送，也不再依赖 Agent 先 `read` 返回 PDF Base64。保留同一 File ID 的沙箱挂载副本。直接上传、引用文件、实际注入的群/话题历史共用此路径；每轮最多直接提供 8 份 PDF，原有附件大小和上下文预算不变。
- 个人助手和数字员工 CLI 默认启用该路径；可设置 `ARKAGENT_PDF_INPUT_MODE=sandbox` 后重启回退旧路径。自行集成 `Gateway` 的调用方需显式传入 `pdfInputMode:"file"` 并提供 `waitForFileActive`；默认兼容旧适配器。文件未就绪、已过期或无法核验时不会提交模型请求，不自动重复上传。
- 合成 PDF 的真实文件引用探针通过，但客户原 PDF 超时/回复“稍后”仍需用原文件验收；既有 Session 里的历史 Base64 不会被此次升级自动删除。手动 `/compact` 的真实原生成功也尚未完成验收。失败时查看错误分类和 Request ID，不因看到 `idle` 就判断任务成功。
- 升级前停止旧服务，备份完整配置目录、数据库及 WAL/SHM、`.credential-key`；回退旧版须使用配套备份，不能让旧程序直接读取迁移后的凭证数据。凭证已在飞书侧轮换时，恢复旧备份可能仍需重新授权。

建议测试顺序：单聊普通消息 → 群聊两人连续 @ → Thread 与引用消息 → 先发 TXT/Markdown/PDF 再 @ → 单聊日历授权 → WebUI 审计。`/new` 会显式切换会话，不作为文件持久性的测试前提。
...
源码开发版保留持久化压缩检查点：重启后的未决手动请求或旧版自动请求先只读查询原事件，不盲目重发；运行结果仍未确定时不会继续投递业务消息。没有未决请求时，普通消息不为压缩阈值额外查询 MA 历史。

压缩成功同时要求本轮正常结束和原生 `agent.thread_context_compacted` 事件，单独 `idle` 或模型声称“已压缩”不算完成。当前手动入口沿用发送 `/compact` 的兼容路径；已观察到平台可能只返回普通文字总结，尚不能保证所有模型/环境都会执行原生压缩。缺少证据时提示“结果尚未确认”，不自动重试并保留原 Session，不能把普通总结当成压缩成功。多 MA 线程的整体压缩确认尚未验收，不把某个子线程的完成事件当作全会话完成。
...
源码开发版会区分已识别的模型文件处理超时（`file_url`）、模型限流和其他 Session 失败；安全的 Request ID 会出现在回复与审计中，不转发上游完整错误正文。执行失败时，流式卡片会尝试用失败提示替换占位/过程文本并关闭流式；如果飞书更新接口也失败，只能报告未确认，不能保证远端卡片已经更新。仅投递确认丢失时保留业务正文以便只读核验，不覆盖成失败结果，也不自动重跑业务。这些改动改善诊断和反馈，不代表已经修复 MA 的 PDF 处理超时。
...
源码开发版补充了**已有Session的附件挂载核查**：再次处理同一来源附件时，已确认的挂载回执可以补回本地挂载标记；上次挂载为未决或结果未知时，先通过MA资源列表核对同一Session、File ID及沙箱路径，唯一匹配后追加`mount_check`成功回执，不修改原失败/未决记录、不重复提交挂载。正常首次挂载和已标记挂载不增加远端查询。查不到、无权限、查询超时或路径有歧义时，只提示“附件挂载结果待核实”，不把空列表当成可安全重发的证明；明确收到HTTP 400 / `InvalidParameter`拒绝的挂载，后续处理仍可复用原File ID重试。此能力不等于启动时自动重放未完成消息；没有Session ID的创建结果、旧版无操作标识的未知上传、模型是否真正读懂PDF，仍不在此恢复保证内。

**新上传的响应丢失恢复**：Gateway在上传前落盘随机操作名，MA Files列表中的文件名会是`arkagent-<UUID>.pdf`等内部名称；飞书展示名称、模型输入中的文件名和沙箱挂载名仍保留原文件名。再次处理结果未知的同一来源附件时，使用有界文件列表查询（最多20页/2,000条、总计4MiB、默认10秒），要求操作名唯一、大小/用途/创建时间匹配，并通过文件详情确认`active`且未过期，才保存`upload_check`并恢复原File ID，不重复下载/上传。同一源消息出现在群背景或话题引用时共用上传记录，其他应用/租户/群/源消息仍隔离。普通成功路径不额外查询。操作名不是官方幂等键，文件元数据也不是服务端内容Hash；旧版记录没有随机操作名，或扫描不完整、无匹配、文件未就绪时，均保留“上传结果待核实”，不按原文件名和大小猜测认领，也不自动重发POST。当前还没有针对这些未知情况的完整管理员强制重传流程。

附件失败记录新增`failure.kind/status/code/requestId`，控制台在阶段状态旁显示；MA HTTP错误保留状态码、经过长度/字符校验的错误码和`x-request-id`，网络错误仅保留固定分类及允许的网络错误码。未知来源错误只记录`unknown`，不会保存上游正文、下载链接或原始cause。错误响应最多读取64KiB、等待最多5秒（或更短的调用方/核查超时）；超限、非JSON、解码失败不产生明确拒绝证明。格式不支持仍保留固定用户提示。这些字段仅供定位，不代表请求可自动重试，也不补造旧记录缺失的信息。
...
默认模式的预置恢复只处理资源绑定，不扫描并重放旧消息。实验持久队列另在用户准备前原子保存原身份、OAuth流程、授权代次或固定预置操作ID；只有这些原始证据完整且未变化，才核查原资源并接回原Inbox任务。初始占位绑定的代次与预置完成回执同事务保存，不能从后来已授权的Credential补造原任务证明。Credential 的秘密值不可回读，因此未知创建仅在原关联标记匹配、列表与详情一致、创建及更新时间相同且时间范围可核实时认领；缺少这些证据就暂停。这不是服务端幂等保证，也不能消除核查后外部管理员同时修改凭证的竞态。
...
- 群聊只处理明确 `@Bot` 的文本消息。
- 个人助手和数字员工单聊均在一个飞书会话中复用 Managed Agents Session；`/compact` 在原 Session 内提交压缩请求（源码版仅手动触发，须有原生事件才确认成功），`/new` 显式重置。数字员工 OAuth 更新已预挂载的 Credential，并校验原 Session 后恢复；源码开发版对无法确认 Vault 挂载的遗留 Session 仅提示显式选择，不再静默交接。普通群聊共享一个排队 Session，Thread 各自共享独立的排队 Session，且群聊仅使用 Bot 身份。
...
若本轮事件明确显示最后一条回复发生在文件读取结束之前，网关不会再把“我来读取”等开场白当最终答案，而是提示“未收到读取后的回复”，保留原Session且不自动重跑。对读后已有文本（例如“稍后分析”），不靠关键词武断判定成功/失败；附件输入会提醒Agent继续处理已返回的文档，没有真实后台任务时不要承诺稍后完成。该提示不覆写Agent/System Prompt，也不是客户PDF问题已经解决的证明；当前客户原场景仍待真实复现与验收。
...
持久化业务队列仍是二次开发的实验选项 `GatewayOptions.durableQueue`，CLI默认未启用。开启前须取得Store运行锁。`recoverPendingMessages()`恢复排队任务并只读核查已派发的未知任务：只有原Session/Agent/配置绑定不变、原运行已结束、此前最终回复接口已确认成功且运行结果指纹一致时，才结束原任务并放行后续消息，不重发原请求或回复。检查点使用版本化加密格式，旧输入记录可读取但不会补造旧回复证明；回退到不识别新格式的代码需配套升级前数据库与密钥备份，不能直接用旧代码打开新数据库。

对于**最终输入已完整准备、但尚未记录模型派发**的普通任务，源码开发版会加密保存原Session、最终输入、提示和上下文回执。重启时核对身份/配置/Session绑定，并通过`GET /sessions/{id}`核实当前Session、Agent和空闲状态（等待最多5秒），然后按原scope顺序继续原任务；不从历史事件推断当前空闲，也不把“新Session还没有事件”当作可执行证明。不重新读取群历史、下载/上传/挂载文件或调用资源配置钩子`buildSessionRequest`。未接入专用生命周期接口时沿用每轮凭证维护钩子；已接入的单聊ready恢复仅执行`userCredentialLifecycle.refresh`，不再调用可能新建资源的`beforeCreateSession`/`beforeDirectTurn`。不同scope的恢复不会等待彼此整轮模型执行；同scope后续消息仍排队。`reconcilePendingMessage()`是二次开发的恢复入口：对这类未派发任务可能继续执行，不是纯只读接口。

源码开发版进一步为实验队列保存**准备步骤计划**：冻结本轮群历史与引用、附件入选范围、已消耗预算、开发者资源选择和最终创建请求。重启后已完成步骤直接复用原结果，不重新取编辑后的群记录，也不会因前8个历史附件已挂载而在同一轮多选第9个。已确认的上传、挂载及Session创建继续复用原资源；响应未知时只按原意图核查，不重复POST。计划最多256步，单步输出2MiB、总计划8MiB，加密格式升级为v3；ready后清理中间计划，保留最终输入检查点。正常路径不新增远端核查。

这个能力仍有明确边界：读取/普通开发者回调已开始但结果未保存时暂停；下载没有可复用缓存且尚无上传意图时，也暂停，不把Hash回执当作已有文件。未执行的自定义回调需要稳定的`sessionConfigurationRevision`才可恢复；已完成回调不重跑。数字员工单聊只有持有原用户授权证明，或原子保存的专用用户准备意图且配置了完整恢复接口时，才恢复准备；旧任务缺证明不能现场补造。旧记录无计划、旧OAuth续跑代次、命令、MA非空闲、Session映射被清除/替换、配置变化或授权等待，都不能绕过安全检查。派发记录先落库；即使随后在真正调用MA前退出，也仍按未知结果核查，不盲重发。最终输入上限仍为2MiB（UTF-8），不扩大附件/上下文预算；CLI仍默认关闭实验队列，不是完整生产恢复承诺。

单聊授权证明绑定 Channel/App/租户/用户、Vault/Credential、OAuth流程ID和加密保存的授权代次。普通Token刷新及同一Credential同步不改变代次；重新OAuth、权限集合变化、绑定资源替换和确认失效会换代。已有绑定恢复时可以刷新原凭证，但不重新申请授权、不重试未知Token交换、不重新预置Vault；代次变了就保留旧任务待处理。默认非持久队列的新消息在确认Token失效并同步占位凭证后仍可按Bot能力执行，通过真实工具权限错误触发OAuth，不要求用户猜关键词。实验队列已冻结准备意图的任务遇到确认失效仍停止，不替旧任务换代。旧Session没有个人Vault仍可使用Bot能力，不为此静默handoff。

二次开发通过`GatewayOptions.userCredentialLifecycle`提供版本化的`prepare`、`refresh`和同步`matches`；第三个可选参数`forDispatch=true`要求凭证当时可使用，而不是仅身份相同。网关向这些回调传快照；`matches`必须返回字面量`true`，Promise不放行。`refresh`须承担恢复所需的安全维护（包括Bot Token），只更新已确认的原Credential，缺少绑定时停止而非重新创建。内置数字员工入口已接入；群聊/Thread不调用个人凭证接口。发送前会在恢复核查、凭证维护后、卡片初始化后再次校验；`ArkClient.run`第六个可选参数是同步`assertBeforeSend`，在历史/SSE等待结束、实际POST前执行，核对原Session映射和授权，异步回调或校验失败不会发送。自定义Ark实现必须执行这个门禁，不能只接受参数后忽略它。这些校验不撤回已经被MA接收的任务，也不保证运行中的工具与外部凭证修改原子隔离。

如需恢复尚未完成的用户凭证准备，必须同时接入同步`capture(message)`、`matchesIntent(message,intent)`和异步`recover(message,intent)`；`prepare`的可选第二参接收已保存的同一意图。`capture`只做本地捕获，不请求远端；Gateway在操作前把意图与pending步骤一次性保存。重启只调用专用`recover`，不重新capture或重跑普通hook，`matchesIntent`同样必须同步返回`true`。原预置journal尚未建立时可沿固定操作ID继续首次创建；已经pending但查不到可靠证据时不得再次POST。生命周期`revision`变化使旧任务暂停，需要明确迁移方案。内置入口使用`employee-credentials-v2`，但不因此启用CLI实验队列。

源码开发版同时为默认模式和实验队列记录 **Session 创建意图**：请求与身份绑定加密保存，再发送创建请求；创建成功后，会话映射、配置与二进制附件挂载回执在同一事务确认。请求保留开发者原生参数和标签，并追加 `arkagent_create_operation`、`arkagent_create_request` 两个保留标签用于核查；它们不是 MA 服务端幂等键，不允许开发者覆盖。响应丢失或本地确认失败时，后续先按 Agent 和时间范围读取完整有界列表，再核对唯一候选的标签及 Session 详情（最多5秒、5页、100项、4MiB），不重复创建。查不到、多候选、配置不能证明、非空闲或配置已变化时保留未决状态，`/new` 不能绕过。旧独立 Session 模式按消息分别记录，维持并行且同消息可找回原创建回执。明确返回 `400 / InvalidParameter` 才记为请求已拒绝，允许修正配置后重新创建；普通超时、网络错误等不按“未创建”处理。

这一步只证明 **Session 资源是否已创建**，不等于旧业务输入已准备完成。实验队列中，只有同时持有可恢复准备计划、原绑定与步骤证据，才会继续准备并最终派发原任务；旧任务没有计划时仍只核实资源。默认非持久队列没有这份准备计划，不能据此声称所有路径均能恢复。普通未知回调、缺失下载内容、缺少原始意图或远端证据的凭证预置，以及未决记录的人工处理入口仍待完成。恢复需要数据库配套的加密密钥，不能只复制数据库或删除密钥后重建。

隔离探针可运行 `node --experimental-strip-types scripts/probe-prepared-recovery.mjs --live <config.env绝对路径> --preparing`：真实创建1个不挂Vault的测试Session，在创建步骤确认后退出，再从原任务继续，向MA仅发送1次随机字符串回显。会消耗MA调用额度，但不连接飞书、不修改Agent或开发机。省略`--preparing`仍测试原有ready后退出窗口；证据目录独立保存，结果未知时不自动重试。

TXT/Markdown 的“原文已提供”回执与二进制挂载分开：只有原文完整进入本轮输入、MA 正常结束且不在授权等待时才确认。历史/引用因预算裁剪、准备失败或运行失败时不误记；ready 检查点保存精确附件 key。压缩后恢复原文也按文件逐一确认，超过单轮256KB预算的来源继续保留待办，不因其他文件恢复成功而一并清除。
...
实验队列也支持普通文本及文本分片的只读核查：发送前保存完整正文指纹，全部消息ID保存后、最终确认前退出时，按原顺序逐条GET核对正文。每条消息必须属于原应用/租户/群或话题、未撤回且为text类型；拼接时不裁剪空白或补换行，完整正文Hash与原MA结果对应的回复意图一致才放行。查询共用5秒等待上限，最多50条，单条正文1MiB、合计4MiB；SDK底层请求仍依赖自身超时，这些不是HTTP响应流的物理中断保证。证明只存消息ID和Hash，不存查询到的正文。Channel SDK的Markdown转post不能冒充文本核查；旧版SDK接入若实际发送text，可走同一核查逻辑。

没有消息ID、仅部分分片发出、旧记录没有发送前正文指纹、SDK流式回退、仍未关闭的流式卡片及准备阶段副作用尚未闭环，不能将其当成完整自动补偿。任一分片缺失、跨话题或正文变化均保留待核查；后续得到完整证明才放行该scope，不自动重发缺失片段。正常回复不新增远程查询，CLI仍未默认开启实验队列。实际读取已有卡片已验证返回更新后正文和关闭状态；文本分片及响应丢失/重启恢复以模拟外部接口的故障注入测试为证，不冒充完整真实飞书E2E。
...
源码开发版在「行为日志」中增加「待处理任务」。二次开发开启持久化队列并将Gateway作为`startEmployeeWeb({ recovery: gateway, ... })`传入后，可查看当前应用/Agent的分页任务状态，重新核查，或明确确认放弃。完整未派发的准备检查点另有「继续已准备任务」按钮，需显式确认后查询空闲并继续执行；不是只读核查，接口action为`resume_prepared`、`confirm: true`。原`reconcile`仍只核查已派发任务，不能用它隐式启动业务。CLI已接入管理视图，但队列仍默认关闭，页面会说明未启用。状态列表不查询MA，不返回用户原文、模型输出或凭证；核查状态只是最近一次记录。
