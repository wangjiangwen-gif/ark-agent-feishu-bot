...
这个能力仍有明确边界：读取/开发者回调已开始但结果未保存时暂停；下载没有可复用缓存且尚无上传意图时，也暂停，不把Hash回执当作已有文件。未执行的自定义回调需要稳定的`sessionConfigurationRevision`才可恢复；已完成回调不重跑。数字员工单聊只有持有原用户授权证明、且配置了专用凭证生命周期接口时才恢复准备；旧任务缺证明不能现场补造。旧记录无计划、旧OAuth续跑代次、命令、MA非空闲、Session映射被清除/替换、配置变化或授权等待，都不能绕过安全检查。派发记录先落库；即使随后在真正调用MA前退出，也仍按未知结果核查，不盲重发。最终输入上限仍为2MiB（UTF-8），不扩大附件/上下文预算；CLI仍默认关闭实验队列，不是完整生产恢复承诺。

单聊授权证明绑定 Channel/App/租户/用户、Vault/Credential、OAuth流程ID和加密保存的授权代次。普通Token刷新及同一Credential同步不改变代次；重新OAuth、权限集合变化、绑定资源替换和确认失效会换代。恢复时可以刷新原凭证，但不重新申请授权、不重试未知Token交换、不重新预置Vault；代次变了就保留旧任务待处理。新发来的消息在确认Token失效并同步占位凭证后仍可按Bot能力执行，通过真实工具权限错误触发OAuth，不要求用户猜关键词。旧Session没有个人Vault仍可使用Bot能力，不为此静默handoff。
...
这一步只证明 **Session 资源是否已创建**，不等于旧业务输入已准备完成。实验队列中，只有同时持有可恢复准备计划、原绑定与步骤证据，才会继续准备并最终派发原任务；旧任务没有计划时仍只核实资源。默认非持久队列没有这份准备计划，不能据此声称所有路径均能恢复。未知回调、缺失下载内容、未完成的凭证预置和未决记录的人工处理入口仍待完成。恢复需要数据库配套的加密密钥，不能只复制数据库或删除密钥后重建。
