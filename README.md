...
当前开发边界：尚未完成部分业务写入后的安全续跑，以及进程退出时普通内存排队消息的持久化重投，后者不能等同于已实现的原授权任务恢复。二次开发调用方应提供授权状态通知回调，并将`onStateChange`接到`Gateway.setAuthorizationWaiting`；在持有Gateway数据库运行锁且Channel可回复后调用 `EmployeeAuthorizationManager.restore()`，在关闭数据库前调用 `close()`。取消并不撤销已发出的外部请求；未知结果不会被伪装成取消成功的外部操作。
