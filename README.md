...
对于**最终输入已完整准备、但尚未记录模型派发**的普通任务，源码开发版会加密保存原Session、最终输入、提示和上下文回执。重启时核对身份/配置/Session绑定，并通过`GET /sessions/{id}`核实当前Session、Agent和空闲状态（等待最多5秒），然后按原scope顺序继续原任务；不从历史事件推断当前空闲，也不把“新Session还没有事件”当作可执行证明。不重新读取群历史、下载/上传/挂载文件或调用资源配置钩子`buildSessionRequest`。原有每轮凭证维护钩子`beforeCreateSession`（旧名称）及单聊凭证检查仍执行。不同scope的恢复不会等待彼此整轮模型执行；同scope后续消息仍排队。`reconcilePendingMessage()`是二次开发的恢复入口：对这类未派发任务可能继续执行，不是纯只读接口。
...
这个能力仍有明确边界：读取/开发者回调已开始但结果未保存时暂停；下载没有可复用缓存且尚无上传意图时，也暂停，不把Hash回执当作已有文件。未执行的自定义回调需要稳定的`sessionConfigurationRevision`才可恢复；已完成回调不重跑。启用`dualIdentity`的数字员工单聊在准备尚未完成时暂不自动恢复，仍需补授权代次校验；已有ready检查点的恢复行为不变。旧记录无计划、旧OAuth续跑代次、命令、MA非空闲、Session映射被清除/替换、配置变化或授权等待，都不能绕过安全检查。派发记录先落库；即使随后在真正调用MA前退出，也仍按未知结果核查，不盲重发。最终输入上限仍为2MiB（UTF-8），不扩大附件/上下文预算；CLI仍默认关闭实验队列，不是完整生产恢复承诺。
...
这一步只证明 **Session 资源是否已创建**，不等于旧业务输入已准备完成。实验队列中，只有同时持有可恢复准备计划、原绑定与步骤证据，才会继续准备并最终派发原任务；旧任务没有计划时仍只核实资源。默认非持久队列没有这份准备计划，不能据此声称所有路径均能恢复。未知回调、缺失下载内容、个人授权代次和未决记录的人工处理入口仍待完成。恢复需要数据库配套的加密密钥，不能只复制数据库或删除密钥后重建。
