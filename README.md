...
源码开发版已支持授权取消、超时与分阶段重启恢复：明确等待用户时继续原轮询，不重发卡片；交换Token或发送卡片结果未知时停止自动重试；已收到的Token先加密落盘，经身份校验后同步原Credential。凭证与同步阶段、取消与任务状态均采用事务更新。仍未完成等待期间队列暂停和多步骤写操作的安全续跑，不能把原请求自动重发视为安全恢复，也不能把自动化测试通过等同于真实飞书端到端验收通过。
...
当前开发边界：尚未完成等待授权期间的单聊队列暂停，以及部分业务写入后的安全续跑。二次开发调用方应提供授权状态通知回调，在持有Gateway数据库运行锁且Channel可回复后调用 `EmployeeAuthorizationManager.restore()`，并在关闭数据库前调用 `close()`。取消并不撤销已发出的外部请求；未知结果不会被伪装成取消成功的外部操作。
